# Управление внешними устройствами через сеть Meshtastic на базе LoRa (в отсутствии мобильного интернета). 
В базовой конфигурации meshtastic не активировано управление внешними устройствами через GPIO.  
  
Сделать это можно несколькими способами, а в статье описан один из них, в результате чего появляется возможность управлять включением различных устройств от светодиода до БПЛА.  Решение "нащупано" самостоятельтно, хотя на оригинальность автор, конечно, не претендует. 
...
**Meshtastic** — проект для создания децентрализованной сети связи с помощью недорогих радиостанций LoRa в отсутствии мобильного интернета. Из коробки позволяет обмениваться текстовыми сообщениями на большие расстояния, здесь мы еще научим ее еще управлять внешними устройствами.

##  Включение GPIO
Почему в  Meshtastic отключена поддержка управления GPIO? Это сделано специально: Meshtastic при создании ориентировался на передаче сообщений в условиях отсутствия интернета. Приоритет состоял в том чтобы ядро  было максимально стабильным, а побочный функционал остался на уровне кастомизации. Не обошлось и без заботы о безопасности,что бы исключить возможность неопытному пользователю подключения нагрузки без должной защиты. В тоже время Meshtastic задумывался как гибкая платформа: если хочется GPIO-контроля, разработчики оставили возможность доработать код самостоятельно.
...
src/ExternalNotificationModule.cpp
.
