...
Meshtastic — проект для создания автономной сети связи на основе недорогих радиостанций LoRa. Позволяет обмениваться сообщениями на большие расстояния в районах без инфраструктуры связи.

Возможности

Дальнобойная связь (рекорд — 331 км)

Работа без телефона и центрального маршрутизатора
...
Возможности на основе GPS-координат

И многое другое

Как это работает

Устройства используют LoRa и формируют mesh-сеть, ретранслируя сообщения.

Сообщения передаются с приложения на радиомодуль через Bluetooth, Wi-Fi/Ethernet или Serial.

Если сообщение не подтверждено другими устройствами, оно пересылается до трёх раз.

Модуль хранит около 30 последних сообщений, заменяя старые новыми.

Mesh-сеть

Mesh — набор узлов с одинаковыми параметрами LoRa: коэффициент расширения, центральная частота, полоса пропускания.

Узел может находиться только в одной сети; для формирования сети все узлы должны иметь одинаковые настройки.

Настройки объединены в пресеты, которые упрощают конфигурацию.

Поверх радио-сети создаются каналы с именем и ключом шифрования. Узлы с одинаковыми настройками канала читают и отображают сообщения.

Один узел может быть в максимум 8 каналах; все узлы сети могут ретранслировать сообщения независимо от настроек каналов.
